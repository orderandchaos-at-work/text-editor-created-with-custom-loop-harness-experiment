...
Tree-sitter-first JavaScript support and the initial JavaScript LSP diagnostics/hover path are implemented and verified. TypeScript/TSX keeps LSP support through the existing `typescript-language-server`, but TypeScript/TSX Tree-sitter parsing is not included.

Implemented:

- `tree-sitter` and `tree-sitter-javascript` are declared in `package.json`.
- `syntaxService.js` detects JavaScript-like files, converts editor lines to parser text, parses supported buffers, collects syntax errors, builds highlight spans, and runs Tree-sitter query searches.
...
1. Detect language from file extension: `.js`, `.jsx`, `.mjs`, and `.cjs`.
...
8. Add rename.
9. Add formatting.
10. Add more grammars only after the JavaScript path remains stable.
...
- Which language should be next after the JavaScript path remains stable.
